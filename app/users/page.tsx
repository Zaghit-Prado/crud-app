...
export default function Home() {
  ...
  // ← isDark y toggleTheme ELIMINADOS (viven en Navbar.tsx)
  ...
  useEffect(() => {
    fetchTasks(); // ← solo esto, sin lógica de tema
  }, []);
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-slate-950 text-slate-900 dark:text-white p-6 md:p-10">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* ← Solo el botón Nueva Tarea (toggle de tema se fue al Navbar) */}
          <button
            onClick={openAdd}
              ...
              <thead>
                <tr className="text-slate-500 uppercase text-xs tracking-wider bg-gray-50 dark:bg-slate-950/50">
                  {["#", "ID", "Título", "Estado", "Creado", "Acciones"].map((h, i) => (
                    <th
                      key={h}
                      className={`px-5 py-3 ${i === 5 ? "text-center" : "text-left"} ${i === 0 ? "w-10" : ""}`}
                    >
                      {h}
                    </th>
                  ))}
